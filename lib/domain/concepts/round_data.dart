...
class RoundData {
  ...
  /// Round key shared by both queue actions.
  /// Format: L{levelId}_S{shortSession}_R{roundNum}  e.g. "L0_S1234_R003"
  String get roundKey {
    final shortSession = sessionId.length > 6
        ? sessionId.substring(sessionId.length - 6)
        : sessionId;
    return 'L${levelId}_S${shortSession}_R${roundNumber.toString().padLeft(3, '0')}';
  }

  /// Queue action for the DETAILED round subcollection.
  ///
  /// Structure: participants/{uid}/rounds/{roundKey}
  ///
  /// A researcher looking up "UID45" in Firebase console will see:
  ///   participants/UID45          ← aggregate summary
  ///   participants/UID45/rounds/  ← subcollection: one doc per decision
  Map<String, dynamic> toQueueAction() {
    return {
      'collection': 'participants',
      'doc': uid,
      'subcollection': 'rounds',
      'subdoc': roundKey,
      'data': toMap(),
    };
  }

  /// Queue action for the FLAT decision document — all decisions for one
  /// participant appear as top-level fields so researchers see everything
  /// without navigating subcollections.
  ///
  /// Structure: participantDecisions/{uid}
  ///   uid: "UID45"
  ///   L0_S1234_R001: { decision, levelId, assetType, cash, … }
  ///   L0_S1234_R002: { … }
  Map<String, dynamic> toParticipantDecisionQueueAction() {
    return {
      'actionType': 'participantDecision',
      ...
      'data': {
        ...
        'speedFlag': speedFlag,
        'decisionTimeSec': (decisionTimeMs / 1000).toStringAsFixed(1),
        'offeredAssetType': offeredAsset.type,
        'offeredAssetPrice': offeredAsset.price,
        'offeredAssetIncome': offeredAsset.income,
        'offeredAssetRisk': offeredAsset.riskLevel,
        'cashBefore': stateBefore.cash,
        'cashAfter': stateAfter.cash,
        'totalAssetsBefore': stateBefore.totalAssets,
        'totalAssetsAfter': stateAfter.totalAssets,
        'totalLoansBefore': stateBefore.totalLoans,
        'totalLoansAfter': stateAfter.totalLoans,
        'assetDied': assetDied,
        'decisionMadeAt': decisionMadeAt.toIso8601String(),
        'conceptsTested': conceptsTested,
        'sessionId': sessionId,
